...
const StatCard = ({ title, value, icon, gradient, description, growth }: StatCardProps) => {
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-start gap-1 mt-1">
            {growth !== undefined && growth !== 0 && (
              <div className="flex items-center gap-1">
                ...
                <span className={`text-xs font-medium ${getGrowthColor(growth)}`}>
                  {formatGrowth(growth)}
                </span>
              </div>
            )}
        ...
        <div className={`p-3 rounded-xl ${getColorClasses(gradient)}`}>{icon}</div>
      </div>
    </Card>
  );
};
...
export const StatsCards = ({ cards, columns = 3 }: StatsCardsProps) => {
  const gridClasses = {
    1: "grid-cols-1",
