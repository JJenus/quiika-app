...
import { Gift, Mail, DollarSign, Lock, Eye, EyeOff } from 'lucide-react';
import { useTransactionStore } from '../../stores/useTransactionStore';
import { LoadingSpinner } from '../ui/LoadingSpinner';
import { ErrorMessage } from '../ui/ErrorMessage';
import type { CreateGiftForm } from '../../types/api';

export const CreateGiftForm: React.FC = () => {
  const [formData, setFormData] = useState<CreateGiftForm>({
    amount: 0,
    email: '',
    isAnonymous: false,
    password: '',
  ...
  const { initializeTransaction, loading, error, clearError } = useTransactionStore();
  ...
  const handleInputChange = (field: keyof CreateGiftForm, value: any) => {
  ...
  return (
    <div className="card p-6 max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="amount" className="block text-sm font-medium text-text-primary dark:text-text-primary-dark mb-2">
            Amount
          </label>
          <div className="relative">
            ...
            <input
              ...
              className={`input-field pl-10 ${formErrors.amount ? 'border-red-500 focus:ring-red-500' : ''}`}
        ...
        <div className="flex items-center">
          ...
          />
          <label htmlFor="anonymous" className="ml-2 text-sm text-text-primary dark:text-text-primary-dark">
            Send anonymously
          </label>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-text-secondary dark:text-text-secondary-dark">
            Add a password to protect your gift from unauthorized claims
          </p>
        </div>
        ...
        >
          {loading.isLoading ? (
            <LoadingSpinner size="sm" text="Initializing Payment..." />
          ) : (
            'Create Gift & Pay'
          )}
        </button>
