...
export const UserManagementPage: React.FC = () => {
  ...
  const handleRoleChange = async (userId: string, newRole: 'SUPER_ADMIN' | 'ADMIN' | 'SUPPORT') => {
  ...
  const canManageUser = (targetUser: any) => {
    ...
    if (currentUser.role === 'ADMIN' && targetUser.role === 'SUPPORT') return true;
    return false;
  };
  ...
  if (loading.isLoading && users.length === 0) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {users.map((user) => (
          <Card key={user.id} className="p-6 hover:shadow-moderate transition-all duration-200">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-text-secondary dark:text-text-secondary-dark">Role</span>
                <select
                  value={user.role}
                  onChange={(e) => handleRoleChange(user.id, e.target.value as any)}
                  disabled={!canManageUser(user) || loading.isLoading}
                  className={`text-xs px-2 py-1 rounded-full font-medium ${getRoleBadgeColor(user.role)} ${
                    canManageUser(user) ? 'cursor-pointer' : 'cursor-not-allowed'
                  }`}
                >
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-text-secondary dark:text-text-secondary-dark">Status</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-text-secondary dark:text-text-secondary-dark">Last Login</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-text-secondary dark:text-text-secondary-dark">Joined</span>
                <span className="text-xs text-text-secondary dark:text-text-secondary-dark">
      ...
      >
        <form onSubmit={handleInviteUser} className="space-y-6">
          ...
          <div>
            <label htmlFor="role" className="block text-sm font-medium text-text-primary dark:text-text-primary-dark mb-2">
              Role
            </label>
            <div className="relative">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
            ...
            <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
              ...
              {inviteForm.role === 'ADMIN' && (
                <>
                  <li>• Full transaction and withdrawal management</li>
                  <li>• User management (Support users only)</li>
