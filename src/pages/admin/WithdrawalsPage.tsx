...
export const WithdrawalsPage: React.FC = () => {
  ...
  const handleReject = async () => {
    if (selectedWithdrawal && rejectReason.trim()) {
      await rejectWithdrawal(selectedWithdrawal.id.toString(), rejectReason);
  ...
  const filteredWithdrawals = mockWithdrawals.filter(withdrawal => {
    ...
    const matchesSearch = searchTerm === '' || 
      ...
      withdrawal.transaction.email.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-text-primary dark:text-text-primary-dark mt-2">
                {formatCurrency(mockWithdrawals
        ...
        {loading.isLoading ? (
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {filteredWithdrawals.map((withdrawal) => (
              <div key={withdrawal.id} className="p-6 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <span className="text-text-secondary dark:text-text-secondary-dark">QUID:</span>
                        <p className="font-mono font-medium">{withdrawal.quid}</p>
                      </div>
                      <div>
                        <span className="text-text-secondary dark:text-text-secondary-dark">Amount:</span>
                        <p className="font-semibold text-success">{formatCurrency(withdrawal.amount)}</p>
                      </div>
                      <div>
                        <span className="text-text-secondary dark:text-text-secondary-dark">Bank:</span>
                        <p className="font-medium">{withdrawal.bank}</p>
                      </div>
                      <div>
                        <span className="text-text-secondary dark:text-text-secondary-dark">Account:</span>
                  ...
                  {withdrawal.status === 'PENDING' && (
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      <Button
                        variant="primary"
                        size="sm"
                        onClick={() => handleApprove(withdrawal.id.toString())}
          ...
          {selectedWithdrawal && (
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
              ...
              <div className="space-y-1 text-sm">
                <p><span className="text-text-secondary dark:text-text-secondary-dark">Account:</span> {selectedWithdrawal.accountName}</p>
                <p><span className="text-text-secondary dark:text-text-secondary-dark">Amount:</span> {formatCurrency(selectedWithdrawal.amount)}</p>
                <p><span className="text-text-secondary dark:text-text-secondary-dark">QUID:</span> {selectedWithdrawal.quid}</p>
              </div>
            </div>
          )}

          <div>
            <label htmlFor="rejectReason" className="block text-sm font-medium text-text-primary dark:text-text-primary-dark mb-2">
